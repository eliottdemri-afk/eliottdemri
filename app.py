...
app = FastAPI(title="API Optimisation Hospitalière", version="3.0")
...
# ============================================================================
# 👉 REMPLACE CETTE LISTE PAR LA TIENNE DEPUIS generate_patient_csv.py
# 👉 Copie-colle exactement ta LISTE_ACTES_AUTORISES ici
...
# ============================================================================
# MAPPINGS SPÉCIALITÉS (AGE, SEXE, DIAGNOSTICS)
# ============================================================================
...
# ============================================================================
# GÉNÉRATION INTELLIGENTE DES PATIENTS (COMME TON CODE)
# ============================================================================

def generer_patients_intelligents(nb_patients: int) -> pd.DataFrame:
    """
    Génère patients de manière réaliste :
    - Choisit code CCAM aléatoire depuis LISTE_ACTES_AUTORISES
    - Mappe vers spécialité via BDD
    - Génère âge/sexe cohérents avec spécialité
    - Choisit DP cohérent
    - PAS de date_souhaitee (sera calculée par l'algo)
    """
    # Filtrer actes disponibles dans la BDD
    ...
    for i in range(nb_patients):
        # Choisir acte CCAM aléatoire
        acte = random.choice(actes_valides)
        
        # Récupérer spécialité via mapping BDD
        spec = CCAM_TO_SPE.get(acte, 'Chirurgie digestive')
        
        # Générer âge cohérent avec spécialité
        age_min, age_max = AGE_PAR_SPECIALITE.get(spec, (30, 75))
        age = random.randint(age_min, age_max)
        
        # Générer sexe cohérent avec spécialité
        ...
        data.append({
            ...
            'acte_ccam': acte,
            'dp': dp
            # PAS de date_souhaitee !
        })
    
    return pd.DataFrame(data)

# ============================================================================
# PRÉDICTION DURÉE DE SÉJOUR (MACHINE LEARNING)
# ============================================================================
...
class AlgorithmeGenetique:
    def __init__(self, patients: List[Patient], config: HospitalConfig, params: AlgoParams):
        ...
        self.t_max_salle = config.t_max_salle * 60  # Convertir en minutes
    ...
    def _est_weekend(self, jour: int) -> bool:
        """Vérifie si le jour est un weekend"""
...
# ============================================================================
# ALGORITHME RECUIT SIMULÉ V4 - OPTIMISATION PLANNING
# ============================================================================
...
class RecuitSimule:
    ...
    def _affecter_ressources(self, solution: Solution):
        """Identique à l'algorithme génétique"""
        for op in solution.operations:
            op.medecin = None
            op.salle = None
            op.jour = None
    ...
    def _generer_voisin(self, solution: Solution) -> Solution:
        """Génère solution voisine par permutation"""
        voisin = solution.copy()
        
        # Permuter 2 opérations aléatoires
...
def root():
    return {
        "message": "API Optimisation Planning Hospitalier",
        "version": "3.0",
        ...
        for op in solution.operations:
            if op.jour is not None:
                ...
                planning.append({
                    ...
                    "date_souhaitee": 0,  # Pas de date souhaitée
                    "ecart_jours": 0
                })
